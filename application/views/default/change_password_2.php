...
<?php if(($admin_user->confirm_admin_user() == true) and isset($ACTION[1]) and preg_match("/^[a-zA-Z0-9]+$/", $ACTION[1])) { ?>
...
<form class="md-float-" autocomplete="Off" id="user_login_form" method="post" action="<?php print $config->base_url(); ?>doAuth/doChangePassword">
...
<span class="alert alert-success">Hello <strong><?php print $admin_user->return_fullname(); ?></strong>, change the password of the Account <strong><?php print $functions->clean_words($ACTION[1]); ?></strong></span>
</div>
...
<input type="hidden" id="username" autocomplete="Off" name="username" class="form-control" value="<?php print $functions->clean_words($ACTION[1]); ?>">
