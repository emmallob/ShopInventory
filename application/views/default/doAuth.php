...
IF(ISSET($SITEURL[1])) {
	...
	function confirm_login($username, $password) {
		...
		if(!empty($username) and !empty($password)) {
			...
			#validate the email 
			if(empty($username)) {
				print "<div style='width:100%' class='btn btn-danger btn-md btn-block'>Sorry! Login Failed. Please enter a Valid Username.</div>";
							...
							if(($results["activated"] == 1) and ($results["status"] == 1)) {
								...
								if($enc_password == $results["password"]) {
									...
									$session->set_userdata(
										ARRAY(
											":storeID" => $results["store_id"],
											":lifeUsername" => $results["username"],
											":lifeID" => $results["id"],
									...
									$automatic_backup = $stores->fetch()->automatic_backup;
									
									#check the automatic backup state
									if($automatic_backup == 1) {
										#create new object and backup the system
										$backup_system = new Stocks;
										#set the file name
										$file = config_item('update_folder')."/db-".date("d-m-Y").".sql";
										#complete database backup
										$backup = $backup_system->backup_system($file);
									}
									print "<script>$(\"#login_user\").attr(\"disabled\", true);</script>";
									#redirect the user
		...
		if(count($result) > 0) {
			foreach($result as $data) {
				...
				if($attempts == ATTEMPTS_NUMBER) {
					$q = "UPDATE _login_attempt SET attempts='$attempts', lastlogin=NOW() WHERE `ip` = '{$functions->get_ip()}' AND `username`='$username'";
					$result = $DB->execute($q);
				} else {
					$q = "UPDATE _login_attempt SET attempts='$attempts' WHERE `ip` = '{$functions->get_ip()}' AND `username`='$username'";
					$result = $DB->execute($q);
				}
			}
		} else {
	...
	IF(($SITEURL[1] == "doLogin") AND ISSET($_POST["login_user_yea"]) AND ISSET($_POST["username"])) {
		...
		if(isset($_POST["login_user_yea"])) {
			if(isset($_POST["username"]) and isset($_POST["password"]) and !isset($_POST["p1"])) {
				#clean the data parsed
	...
	IF(($SITEURL[1] == "doFirstChange") AND ISSET($_POST["change_password_first"])) {
		if(isset($_POST["change_password_first"])) {
			if(isset($_POST["password1"]) and ($session->userdata(":lifeUsername")) and isset($_POST["password2"])) {
				#clean the data parsed
				...
				if($password1 != $password2) {
					...
					$DB->query("insert into _activity_logs set full_date=now(), store_id='".STORE_ID."', date_recorded=now(), admin_id='$username', activity_page='password', activity_id='$username', activity_details='$username', activity_description='You changed your password from the default <strong>1234567</strong>.'");
					#reload the page
					redirect( $config->base_url() . 'dashboard', 'refresh:2000');
				}
			} else {
